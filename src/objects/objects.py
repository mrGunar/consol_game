...
class AliveObject:
    ...
    def step(self, x: int, y: int) -> None:
        if (
            0 < self.x + x < MapConfig.MAP_HEIGHT.value - 1
            and 0 < self.y + y < MapConfig.MAP_WIDTH.value - 1
        ):
            self._coordinates = Coordinate(
                self._coordinates.x + x, self._coordinates.y + y
            )
...
class WeaponObject:
    ...
    def change_coords(self, new_x, new_y):
        self._coordinates = Coordinate(
            self._coordinates.x + new_x, self._coordinates.y + new_y
        )
